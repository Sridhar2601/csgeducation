...
// // course with dropdown but not mbl responsive
// import { useState, useEffect } from 'react';
// import { Link, useLocation } from 'react-router-dom';
// import { motion, AnimatePresence } from 'framer-motion';
// import './Header.css';
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="flex items-center space-x-2">
            ...
            <div className="font-heading font-bold text-xl text-gray-900">CSG Education</div>
          </Link>
          ...
          <ul className="hidden md:flex space-x-6 items-center">
                ...
                <ul className="absolute left-0 mt-2 w-56 bg-white shadow-lg rounded-md py-2 z-50">
                  {courses.map((cat, idx) => (
                    <li
                      ...
                      onMouseLeave={() => setOpenSubmenu(null)}
                    >
                      <span className="flex justify-between items-center px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer">
                        {cat.title} <span>▸</span>
                      </span>
                      {openSubmenu === cat.title && (
                        <ul className="absolute left-full top-0 mt-0 ml-1 w-56 bg-white shadow-lg rounded-md py-2 z-50">
            ...
            <li><Link to="/success" className="nav-link">Success Story</Link></li>
              ...
              <div className="px-2 pt-2 pb-3 space-y-1 bg-white">
                ...
                <div>
                  {courses.map((cat, idx) => (
                    <div key={idx}>
                      <button
                        className="w-full flex justify-between items-center px-3 py-2 text-base font-medium text-gray-700 hover:bg-gray-50"
                        onClick={() =>
                          setOpenDropdown(openDropdown === cat.title ? null : cat.title)
                        }
                      >
                        {cat.title}
                        <span>{openDropdown === cat.title ? "▾" : "▸"}</span>
                      </button>
                        ...
                        <div className="ml-4">
                          {cat.topics.map((topic, i) => (
                            <Link
                              key={i}
                              to={`/course/csg-${topic.toLowerCase().replace(/\s+/g, "-")}`}
                              className="block px-3 py-1 text-gray-600 hover:bg-gray-100"
                            >
                              {topic}
                            </Link>
                          ))}
                ...
                <Link to="/about" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-50">
                  About
                </Link>
                <Link to="/success" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-50">
                  Success Story
                </Link>
